...
-- Scrape Data policies
ALTER TABLE scrape_data ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own scrape data"
  ON scrape_data FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can create own scrape data"
  ON scrape_data FOR INSERT
  WITH CHECK (auth.uid() = user_id);
...
-- Cleanup old scrape data (runs daily)
